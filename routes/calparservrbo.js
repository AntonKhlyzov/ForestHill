...
// Function to fetch iCal data (from VRBO API or file)
async function fetchVRBOICalData(icalUrl) {
    try {
...
function parseVRBOICalData(data) {
    ...
    lines.forEach(line => {
        if (line.startsWith('DTSTART')) {
            ...
            //console.log('Parsed VRBO DTSTART:', startDate);
        } else if (line.startsWith('DTEND')) {
            endDate = extractDate(line, true); // Pass a flag to adjust DTEND
            //console.log('Parsed VRBO DTEND:', endDate);
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        //console.log(`Processing VRBO event from ${startDate} to ${endDate}`);
        ...
        while (currentDate <= endDate) {
            //console.log('Adding VRBO disabled date:', formatDate(currentDate));
        ...
        if (i + 1 < events.length) {
            ...
            if (isSameDay(addDays(endDate, 1), nextStartDate)) {
                //console.log('Adding disabled VRBO date due to back-to-back event:', formatDate(nextStartDate));
